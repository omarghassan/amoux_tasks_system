...
function populateToDoSection(todoTasks) {
    ...
    todoTasks.forEach(task => {
        ...
        // Update dropdown to include edit option
        updateDropdownMenu(taskElement, task);
...
function updateDropdownMenu(taskElement, taskData) {
    const dropdownMenu = taskElement.querySelector('.status-dropdown-menu');
    
    // Add edit task option to the dropdown menu
    const editTaskItem = document.createElement('li');
    editTaskItem.innerHTML = `
        <button class="dropdown-item edit-task-item d-flex align-items-center" 
                type="button" 
                data-task-data='${JSON.stringify(taskData).replace(/'/g, "&#39;")}'>
            <i class="ri-edit-line me-2"></i>
            Edit Task
        </button>
    `;
    
    // Insert edit option after the header but before status options
    const headerItem = dropdownMenu.querySelector('li:first-child');
    if (headerItem && headerItem.nextSibling) {
        dropdownMenu.insertBefore(editTaskItem, headerItem.nextSibling);
    } else {
        dropdownMenu.appendChild(editTaskItem);
    }
    
    // Add click handler for edit button
    const editButton = editTaskItem.querySelector('.edit-task-item');
